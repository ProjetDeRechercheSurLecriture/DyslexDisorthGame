...
OPrime.reportBug = function(message) {
  if (OPrime.isAndroidApp()) {
    Android.sendDevEmail(message);
  } else {
    alert(message);
  }
};
...
/**
 * Recursive function to drill down into an object to find the path to a
 * specific node and assign value to that node.
 * 
 * @param rootNode
 *          is the highest node object, containing all children
 * @param nodeArray
 *          is an array of strings naming the nodes to cycle through
 * @param value
 *          is the value to assign to resultant embedded object within the
 *          rootNode
 * @returns null if complete, otherwise returns function call to itself
 */
OPrime.setChild = function(rootNode, nodeArray, value) {
  OPrime.debug(rootNode, nodeArray);
  var nextNode;
  if (nodeArray.length == 1) {
    nextNode = nodeArray.shift();
    rootNode[nextNode] = value;
    return;
  } else {
    nextNode = nodeArray.shift();
    return OPrime.setChild(rootNode[nextNode], nodeArray, value);
  }
};
